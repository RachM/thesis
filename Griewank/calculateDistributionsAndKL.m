...
nKernel = 1000;
pKernel = zeros(length(dim), nKernel);
xKernel = zeros(length(dim), nKernel);
bandwidths = zeros(length(dim));
kl = zeros(length(dim));
...
for i=1:length(dim)
    ...
    disp(['Finished Griewank: ', num2str(dim(i))]);
