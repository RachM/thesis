...
dataKL = load('distances_kl.mat');
distKL = dataKL.distances + dataKL.distances';
dataNCD = load(['distances_', type,'.mat']);
distNCD = dataNCD.distances;

totalProblems = size(distNCD, 1);

dKL = zeros((totalProblems - 1) * (totalProblems - 2) / 2, 1);
dNCD = zeros((totalProblems - 1) * (totalProblems - 2) / 2, 1);
ctr = 1;

for ind1=1:totalProblems
    for ind2=(ind1+1):(totalProblems-1)        
        dKL(ctr) = distKL(ind1, ind2) + distKL(ind1, ind2)';
